...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
    ...
    TestCppWrapper *cppWapper = [[TestCppWrapper alloc] init];
    //[cppWapper testPrint];
    [cppWapper setCallback];
    [cppWapper testCallback];
    
    OscCppWrapper *oscCppWapper = [[OscCppWrapper alloc] init];
    [oscCppWapper setCallback];
    [oscCppWapper setDelegate:self];
    [oscCppWapper setUp:12345];
...
- (void)hueOscMessageGet:(int)lihgtId hue:(int)hue sat:(int)sat value:(int)value{
    // NSLog( @"hueOscMessageGet lihgtId%d, hue %d, sat %d, value %d", lihgtId, hue, sat, value);
    ...
    for (PHLight *light in cache.lights.allValues) {
        ...
        // [lightState setHue:[NSNumber numberWithInt:(int)((float)(hue/360.0) * 65535.0)]];
        [lightState setHue:[NSNumber numberWithInt:hue]];
        [lightState setBrightness:[NSNumber numberWithInt:value]];
        [lightState setSaturation:[NSNumber numberWithInt:sat]];
            
        // NSLog( @"hueOscMessageGet sending! lihgtId%d, hue %d, sat %d, value %d", lihgtId, hue, sat, value);
